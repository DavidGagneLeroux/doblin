...
#' @param time_threshold time point threshold (keep barcodes with at least x non-zero time points)
#' @param name sample's name
#' @return a matrix containing the ID,
#' frequency at each time point, average frequency and duration of each remaining barcode
#' @export filterData

filterData <- function(input_df, time_threshold, name){
  ...
  ##TODO: Should we still use a "flag" to remove one additional column (in case of low CFU)?
  ## (see  src script)
  ...
  ##TODO: Do we let the user define the MIN mean frequency

  sample.clustering = mat[mat$points>time_threshold & mat$mean>5e-5,]
