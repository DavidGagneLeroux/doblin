...
The *doblin* package is intended to be a comprehensive toolbox to understand how clonal populations of cells evolve over time. It was developed for the analysis of dominant barcode lineage data resulting from *Next Generation Sequencing* (NGS). In combination with functionalities needed for data analysis, it offers a variety of visualizations.

## 1.1 Barcode extraction from the FASTQ/FASTA file and determining putative “true” lineages

This subsection is specifically for users with sequencing data who haven’t yet perform barcode extraction. Prior to analysis, you must identify and extract the barcode sequences from your raw sequencing data. *Bartender* is a usefull tool to process barcode data (https://github.com/LaoZZZZZ/bartender-1.1). 


At this point, it is primordial to format your data as follows:
...
Time: integer representing the time at which the data was measured. If the file contains time points starting at 0, we extract the associated values to produce an initial barcode library. If the user doesn't have an initial barcode library, the time points should at least start at t=1.

Reads: number of barcodes that were computed at a certain time for a certain consensus sequence.

## 1.2 Dependencies

If the necessary packages aren't already installed on your system, you should install them before running the following pipeline. 
...
In order to use this pipeline, you must run a command line of the following form: `Rscript doblin/demo/main.R -x 1000 -t 0.0005 -o {output_directory} -s {sample_name} -i {sample_file} -c 14`.

Example: 

`Rscript ./main.R -x 1000 -t 0.0005 -o ~/Documents/ -s M1,M2 -i ./M1_input.csv,./M2_input.csv -c 14`
...
To compare barcode trajectories within and between samples, we aimed to use consistent color coding for barcode lineages. First, we assigned a unique color to all lineages that reached a relative frequency - specified by the `-t` argument given by the user (default: 0.0005). For example, a barcode lineage that was assigned the color “magenta [#c20078]” will always have this color in all the figures. Conversely, no other barcode was assigned the same color. To create the Muller-type plots, the barcode frequencies at every time point were represented in linear scale. For each sample, the barcodes are sorted by the maximum frequency they attained over the time-series. This produced a stacked area plot where dominant barcodes are shown starting from the bottom of the panel and progressively lower-frequency barcodes are shown at the top. The same data was used to plot the frequency trajectories in log10-transformation. Barcodes that reached the minimum frequency (`-t`)  throughout its time-series are shown in color, whereas the remaining barcodes are shown in grey for clarity.
...
## 2.4 Spearman Correlation (for dataset containing an initial library)

If the user provided an initial library, he has the possibility to extend his analysis by quantifying the evolution of his dataset compared to the given library. First, we create a plot in order to visualize the barcode frequencies from the initial library. Another useful tool is the scatter plot we use to visualize the correlation between the barcode frequencies at all time points and the barcode frequencies at time 0. The Jaccard similarity index, which compares members from two sets to see which members are shared and which are distinct, can also be computed. The higher the percentage of that index, the more similar the two sets are. 
...
for(i in 1:length(input_data)){
  if (any(input_data[[i]]$Time == 0)){

    ## 7.1: Plot the barcode frequencies from the initial library.
    initial_barcode_library <- input_data[[i]][input_data[[i]]$Time == 0,]
    input_data[[i]] = input_data[[i]][input_data[[i]]$Time != 0,]
    ...
    ## 7.2: Create a scatter plot
    #plotCorrelation(input_data[[1]], barcode_library, cohort_names[[1]])
    ...
    #TODO: Do we really want one Jaccard plot per sample? It doesn't make sense to move the function outside
    # the "if" condition
    plotJaccardIndex(top_final[i], top_max[i], initial_barcode_library_topN, cohort_names[[i]])
  }
}

```

Finally, we compute and plot the Spearman's correlation between a given sample and the initial library for all time points.
...
To identify the clonal lineages, we cluster the barcode lineages for each sample based on the similarity of their time series behavior. To maximize the accuracy of this clustering, we exclude barcodes with insufficient time points (the `-c` argument). This ensures that all barcode lineages included in the clustering have a sufficient number of points for pairwise comparison. This procedure means that the lineage clustering focuses on dominant and persistent clones; barcodes that immediately went to extinction were excluded. 

We compute the distance matrix according to the clustering method given by the user - Pearson or dtw - and perform a hierarchical clustering. The user can choose a linkage method among those presented in the r documentation (https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/hclust). As an example, the “average” agglomerative clustering method implements the algorithm Unweighted Pair Group Method with Arithmetic mean (UPGMA). This method computes the distance between two clusters as the arithmetic mean of the distances between all lineages in both clusters. Then, for each cluster, we fit a consensus trajectory using the local regression (loess). Loess is a form of moving average where a line is fit locally using neighboring points weighted by their distance from the current point.

To determine the optimal clustering threshold, we note two general trends. First, the loess of clusters with very few lineages will be sensitive to sequencing error. Second, when the threshold is too small, there are many clusters, but multiple clusters are similar to each other. Third, when the threshold is too large, there are very few clusters where barcodes with distinct dynamics are grouped together. In clustering, the practice is to find the cross-over between the smallest distance between cluster centroids (our loess average) and the number of clusters.
...
cat("Enter a linkage method (methods from
...
for(i in 1:length(filtered_dataframes)) {
  clusters_df = get_clonal_lineage_clusters(filtered_dataframes[[i]], cohort_names[[i]],
                                            linkage, clustering, covariance)
...
for(i in 1:length(filtered_dataframes)) {
  ...
  selected_clusters = get_clonal_lineage_clusters(filtered_dataframes[[i]],
                                                  cohort_names[[i]],linkage, clustering,
                                                  covariance, selected_threshold)
